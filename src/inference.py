...
import librosa
from utils import ls, preprocess_wav, to_numpy, plot_mel_transfer_infer, reconstruct_waveform, inference_hifigan
...
parser.add_argument("--use_hifigan", type=bool, default=True, help="Use Hifi-GAN Vocoder")
parser.add_argument("--hifigan_checkpoint", type=str, default="./hifigan_checkpoint/checkpoints_hifigan_universal_ptbr/g_03700000", help="HiFi-GAN checkpoint")
parser.add_argument("--hifigan_config", type=str, default="./hifigan_checkpoint/checkpoints_hifigan_universal_ptbr/config.json", help="HiFi-GAN config path")
...
def audio_infer(wav):
    ...
    if opt.use_hifigan:
        print('Reconstructing with Hifi-GAN...')

        with open(opt.hifigan_config) as f:
            data = f.read()
        json_config = json.loads(data)
        h = AttrDict(json_config)
        # Denormalization - To be able to use as input to the HiFi-Gan model
        spect_trg = spect_trg*(max_value-min_value) + min_value
        resample_factor = h.VOC_sampling_rate/h.TTS_sampling_rate
        spectrogram = torch.nn.functional.interpolate(torch.FloatTensor(spect_trg).unsqueeze(0).unsqueeze(0), scale_factor=(1, resample_factor), mode='bilinear', align_corners=True).squeeze(0)
        audio = inference_hifigan(mel=spectrogram, checkpoint_file=opt.hifigan_checkpoint, config=h, device=device)
        write(root+'/gen/%s_gen.wav'%fname, h.VOC_sampling_rate, audio)
    else:
